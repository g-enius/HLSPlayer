...
#define DownloadFolder [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask,YES) objectAtIndex:0] stringByAppendingPathComponent:kPathDownload]

@implementation VideoDownloader
@synthesize totalprogress,playlist,delegate,downloadPart,currentProgress, clearCaches, indexPath;
...
-(void)startDownloadVideo
{
    ...
    if(downloadArray == nil)
    {
        ...
        for(int i = 0;i< self.playlist.length;i++)
        {
            ...
            SegmentDownloader* sgDownloader = [[SegmentDownloader alloc]initWithUrl:segment.locationUrl andFilePath:playlist.uuid andFileName:filename];
...
//    NSLog(@"cleanDownloadFiles");
    ...
    NSString *savePath = [DownloadFolder stringByAppendingPathComponent:self.playlist.uuid];
    savePath = [DownloadFolder stringByAppendingPathComponent:self.playlist.uuid];
...
-(NSString*)createLocalM3U8file
{
    if(playlist !=nil)
    {
        NSString *downloadDic = DownloadFolder;
